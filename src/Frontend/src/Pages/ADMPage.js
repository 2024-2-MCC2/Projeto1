...
import { getAllNoticias, createNoticia, updateNoticia, deleteNoticia } from '../Service/NoticiasAPI';
...
function ADMPage() {
    ...
    const handleCreate = async (newNoticia) => {
        await createNoticia(newNoticia);
        const updatedNoticias = await getAllNoticias();
        setNoticias(updatedNoticias);
    };

    const handleUpdate = async (id, updatedNoticia) => {
        await updateNoticia(id, updatedNoticia);
        const updatedNoticias = await getAllNoticias();
        setNoticias(updatedNoticias);
    };
...
*/
    ...
    */
    return (
        <Container>
            <ADM NoticiasData={noticias} onCreate={handleCreate} onUpdate={handleUpdate} onDelete={handleDelete} />
        </Container>
    )
}
